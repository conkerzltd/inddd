...
BEGIN
  ...
  IF v_ticket.status IN ('REMOTE_BOOKED', 'LINK_SENT', 'INSIDE_WAITING') THEN
    ...
    v_result.expected_window_start := v_eta_anchor + (v_result.eta_min_minutes || ' minutes')::interval;
    v_result.expected_window_end := v_eta_anchor + (v_result.eta_max_minutes || ' minutes')::interval;
