...
-- Fix AFTER_CURRENT: place ticket BEFORE all INSIDE_WAITING (position #1)
...
BEGIN
  ...
  IF p_insert_position = 'AFTER_CURRENT' THEN
    -- Place BEFORE all INSIDE_WAITING tickets (position #1)
    SELECT MIN(rank_key) INTO v_min_rk
    FROM tickets
    WHERE clinic_id = v_ticket.clinic_id AND visit_date = v_today AND status = 'INSIDE_WAITING';

    IF v_min_rk IS NOT NULL THEN
      v_new_rk := v_min_rk - 1000;
    ELSE
  ...
  -- Compute new rank_key if anchor found (AFTER_N / END cases)
...
-- Also fix set_urgent_and_insert for consistency
