...
DECLARE
  ...
  v_rate numeric;
  v_inside_load numeric := 0;
  v_prearrival_load numeric := 0;
...
BEGIN
  ...
  CASE v_ticket.status
    ...
    WHEN 'INSIDE_WAITING' THEN
      ...
      -- Base ETA set here, will be updated below with remote load
  ...
  -- ── ETA calculation with integrated pre-arrival load ──
  IF v_ticket.status IN ('REMOTE_BOOKED', 'LINK_SENT', 'INSIDE_WAITING') THEN
    ...
    -- Count inside-clinic patients ahead (for REMOTE_BOOKED/LINK_SENT; INSIDE_WAITING already has v_position)
    ...
    -- Inside load = (inside_count_ahead) * avg
    v_inside_load := (v_position - 1) * v_avg;

    -- Count pre-arrival tickets ahead (all sources, no link-activation filter)
    ...
    -- Weighted pre-arrival load
    v_rate := GREATEST(0, LEAST(1, COALESCE(v_clinic.remote_showup_rate, 0.6)));
    v_prearrival_load := v_remote_count * v_rate * v_avg;

    -- Total load = inside + weighted pre-arrival
    v_total_load := v_inside_load + v_prearrival_load;

    -- Compute final ETA range
    v_result.eta_min_minutes := GREATEST(1, (v_total_load * 0.7)::int);
    v_result.eta_max_minutes := GREATEST(1, (v_total_load * 1.3)::int);
    v_result.expected_window_start := now() + (v_result.eta_min_minutes || ' minutes')::interval;
    v_result.expected_window_end := now() + (v_result.eta_max_minutes || ' minutes')::interval;
