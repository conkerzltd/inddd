...
-- Add "draft" to entity_status enum
ALTER TYPE public.entity_status ADD VALUE IF NOT EXISTS 'draft' BEFORE 'pending';

-- Update onboard_clinic to create clinics with "draft" status
CREATE OR REPLACE FUNCTION public.onboard_clinic(
  p_name_ar text,
  p_primary_specialty_id uuid DEFAULT NULL,
  p_governorate_ar text DEFAULT 'placeholder',
  p_locality_level2_ar text DEFAULT 'placeholder',
  p_locality_level2_type text DEFAULT 'CITY'::text,
  p_locality_level3_ar text DEFAULT NULL::text,
