...
-- 1) Add CHECK constraint: rank_key NOT NULL when INSIDE_WAITING
ALTER TABLE public.tickets
ADD CONSTRAINT chk_rank_key_inside_waiting
CHECK (status <> 'INSIDE_WAITING' OR rank_key IS NOT NULL);

-- 2) Extend return type with expected_window fields
ALTER TYPE public.patient_queue_view
ADD ATTRIBUTE expected_window_start timestamptz,
ADD ATTRIBUTE expected_window_end timestamptz;
...
BEGIN
  ...
  CASE v_ticket.status
    ...
    WHEN 'INSIDE_WAITING' THEN
      ...
      -- Deterministic position with tie-breakers
