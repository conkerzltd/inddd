...
-- =============================================
-- Step 5: Session Controls + Reinsertion + Urgent Insert
-- =============================================

-- 1) set_session_paused
CREATE OR REPLACE FUNCTION public.set_session_paused(p_clinic_id uuid, p_paused boolean)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_clinic.session_paused = p_paused THEN
    RETURN jsonb_build_object('changed', false, 'session_paused', p_paused);
  END IF;

  UPDATE clinics SET session_paused = p_paused WHERE id = p_clinic_id;

  v_action := CASE WHEN p_paused THEN 'PAUSED' ELSE 'RESUMED' END;

  INSERT INTO audit_log (clinic_id, action, actor_id, details)
  VALUES (p_clinic_id, v_action, v_uid, jsonb_build_object('session_paused', p_paused));

  RETURN jsonb_build_object('changed', true, 'session_paused', p_paused);
END;
$$;

-- 2) set_intake_open
CREATE OR REPLACE FUNCTION public.set_intake_open(p_clinic_id uuid, p_open boolean)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  IF v_clinic.intake_open = p_open THEN
    RETURN jsonb_build_object('changed', false, 'intake_open', p_open);
  END IF;

  UPDATE clinics SET intake_open = p_open WHERE id = p_clinic_id;

  v_action := CASE WHEN p_open THEN 'INTAKE_OPENED' ELSE 'INTAKE_CLOSED' END;

  INSERT INTO audit_log (clinic_id, action, actor_id, details)
  VALUES (p_clinic_id, v_action, v_uid, jsonb_build_object('intake_open', p_open));

  RETURN jsonb_build_object('changed', true, 'intake_open', p_open);
END;
$$;

-- 3) reinsert_returned — midpoint insertion algorithm
CREATE OR REPLACE FUNCTION public.reinsert_returned(
  p_ticket_id uuid,
  p_insert_position insert_position,
  p_insert_n int DEFAULT NULL,
...
BEGIN
  ...
  IF p_insert_position = 'AFTER_CURRENT' THEN
    -- Anchor = IN_SERVICE ticket's rank_key
    ...
    IF v_anchor_rk IS NULL THEN
      -- Fallback: lowest INSIDE_WAITING
      SELECT id, rank_key INTO v_anchor_id, v_anchor_rk
      FROM tickets
  ...
  -- Compute midpoint if anchor found
...
-- 4) set_urgent_and_insert — reuse midpoint insertion
CREATE OR REPLACE FUNCTION public.set_urgent_and_insert(
  p_ticket_id uuid,
  p_insert_position insert_position,
  p_insert_n int DEFAULT NULL,
  p_note text DEFAULT NULL
...
BEGIN
  ...
  -- Resolve anchor (same algorithm as reinsert_returned)
  v_new_rk := NULL;
  v_anchor_id := NULL;
  ...
  IF p_insert_position = 'AFTER_CURRENT' THEN
    ...
    IF v_anchor_rk IS NULL THEN
      v_new_rk := 1000000000 + (EXTRACT(epoch FROM now()) * 1000)::bigint;
    END IF;
